import type {UserAgentResponseOptions, WebResponse} from './types.js';
...
export class UserAgentResponse {
  ...
  static fromWeb(res: WebResponse): UserAgentResponse {
    const headers = UserAgentHeaders.fromWeb(res.headers);
    return new UserAgentResponse({body: res.body, headers, statusCode: res.status, statusMessage: res.statusText});
  }
